...
// Razorpay is initialized client-side only
...
const MOCK_MODELS = {
  "1": {
    ...
    title: "Model 01",
    description:
      "This is a detailed 3D model of a spherical object with bubbles.",
...
const PaymentPage = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      // For demo purposes, we'll simulate a successful payment without a real order_id
  ...
  const handlePayment = () => {
    ...
    if (!razorpayKeyId) {
      ...
      setRazorpayError("Payment configuration error. Please contact support.");
      setPaymentProcessing(false);
      return;
    }
    ...
    // In test mode, we can skip the order_id requirement
    const options = {
      key: razorpayKeyId,
      amount: amount,
      currency: "INR",
      ...
      // Removed order_id to avoid validation errors in test mode
      handler: function (response: any) {
        ...
        setTimeout(() => {
          router.push(
            `/order-confirmation/${id}?orderId=${response.razorpay_payment_id}`
          );
        }, 2000);
      },
      prefill: {
    ...
    try {
      ...
      let errorMessage =
        "Payment gateway encountered an error. Please try again.";
      ...
      if (error.message) {
        ...
        if (
          ...
          errorMessage =
            "Invalid payment amount. Please try again with a valid amount.";
        ...
        } else if (error.message.includes("otp-credentials")) {
          // Handle the specific otp-credentials error shown in logs
          errorMessage =
            "Authentication error. This is a test mode limitation. In production, this would be handled by the server.";
        }
      }

      setRazorpayError(errorMessage);
      setPaymentProcessing(false);
    }
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta
          name="description"
          content="Complete your payment for 3D printing"
        />
      </Head>
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-8">
            ...
            <div className="flex items-start space-x-4 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Estimated Print Time: {time || "8"} Hrs
                </p>
              </div>
            </div>
            ...
            <div className="border-t pt-4">
              ...
              <div className="flex justify-between mb-2">
                ...
                <span>
                  ₹ {Math.round((Number(price) || model.basePrice) * 0.18)}
                </span>
              </div>
              ...
              <div className="flex justify-between font-bold text-lg mt-4 pt-4 border-t">
                ...
                <span>
                  ₹{" "}
                  {Math.round((Number(price) || model.basePrice) * 1.18 + 100)}
                </span>
              </div>
            </div>
          </div>
